		...
		<div class="col-md-8 col-md-offset-2 col-sm-6 col-sm-offset-3">
			<div class="panel panel-default mt-xl">
				...
				<div class="panel-body">
					{!! Form::open(['url' => 'profile/create', 'role' => "form", 'data-parsley-validate', 'novalidate', 'class' => 'form-horizontal mb-lg']) !!}
					{!! Form::token() !!}
					...
					<div class="row">
						...
						<div class="mb col-md-6 {{ $errors->has('blood_type') ? ' has-error' : '' }}">
							...
							{!! Form::select('user_type', ['patient'=>'ผู้ป่วย', 'administrator'=>'ผู้ดูแลระบบ', 'staff'=>'เจ้าหน้าที่', 'doctor'=>'แพทย์', 'nurse'=>'พยาบาล', 'pharmacist'=>'เภสัชกร'], old('blood_type'), ['class' => 'form-control', 'required', 'id'=>'user_type']) !!}
							<input type="hidden" id="user_tt" name="patient" value="patient" />
						</div>
					</div>
					<div class="row" id="doctor_info" style="{{ old('user_type') == 'doctor' ? '' : 'display:none;' }}">

						<div class="mb col-md-6 {{ $errors->has('doctor_number') ? ' has-error' : '' }}">
							{!! Form::label('doctor_number', 'รหัสประจำตัวแพทย์', ['class' => 'control-label']) !!}
							{!! Form::text('doctor_number', old('doctor_number'), ['class' => 'form-control', 'required']) !!}
						...
						<div class="mb col-md-6 {{ $errors->has('department') ? ' has-error' : '' }}">
							...
							{!! Form::text('department', old('phone_number'), ['class' => 'form-control', 'required']) !!}
					...
					<div class="row">
						...
						<div class="col-md-4 col-md-offset-4 mt-xl">
							<div class="form-group">{!! Form::submit('สร้างบัญชีผู้ใช้งาน', ['class' => 'btn btn-primary btn-block']) !!}</div>
						</div>
					</div>
